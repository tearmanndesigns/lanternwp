...
Accessibility (WCAG 2.1 AA) Every release must pass the Tearmann Verified accessibility checklist in full — semantic landmarks, keyboard navigation, contrast ratios, visible focus states, and properly labeled forms are non-negotiable, not optional polish.

Performance (Core Web Vitals) Every release must meet LCP < 2.5s, INP < 200ms, and CLS < 0.1 on mobile. Features that compromise these thresholds get redesigned or cut, not shipped with a caveat.
